...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        tdoc.Add("Set", TrussDocument::Array());
        auto& set = tdoc["Set"];
        for (auto& doc: result)
        {
            if (doc.Keyword == "NODE")
            ...
            {
                auto plane = GetPlane(doc);
                tdoc.Add("Element", plane);
            }
            else if (doc.Keyword == "SET_NODE_LIST_TITLE")
            {
                auto nodeset = GetNodeSet(doc);
                nodeset["key"] = (int)set.Count();
                set.Add(nodeset);
            }
        }
